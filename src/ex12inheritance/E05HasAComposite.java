...
: X has a Y => X가 Y를 소유(포함)하다. 여기서는 권총을 소유한 경찰을 표현

Has-A와 같이 소유의 관계를 상속으로 표현하면 클래스간의 강한 연결고리가 형성되어 권총이 없는 경찰을 표현하기 힘들어진다.
...
class Police {
	...
	public Police(int bNum, int hCuff) {
		...
		참조변수가 null을 할당 받았다는 것은 참조할 인스턴스가 없는 것을 의미한다.
		즉 힙영역에 생성된 인스턴스가 아예 없으므로 소유하지 않는 것을 표현할 수 있다.
		 */
		if (bNum <= 0) {
			gun = null;
		} else {
...
public class E05HasAComposite {
	...
	public static void main(String[] args) {
		// 권총을 보유한 경찰로 총알 5발, 수갑 3개로 인스턴스 생성
