...
오버라이딩 : 메서드명, 매개변수, 반환타입까지 모두 동일한 메서드를 상속관계에 있는 하위 클래스에서 재정의 한 것을 말한다.
	이 경우 하위클래스에서 정의한 매서드가 상위클래스의 메서드를 가리게 되므로, 항상 최하위 클래스의 메서드가 호출된다.
...
class C extends B {
	/*
	rideMethod()는 오버라이딩 된 메서드로 상위클래스에 정의된 멤버메서드와
	반환타입, 메서드명, 매개변수의 갯수 및 타입이 모두 동일하다.
	즉 동일한 메서드이다.
	 */
	@Override
	public void rideMethod() {
	...
	/*
	loadMethod()는 오버로딩 된 메서드로 상위클래스에 정의도니 멤버메서드와 이름만 동일할 뿐
	매개변수의 타입, 개수 등이 다르게 정의된다.
	따라서 다른 메서드이다.
	 */
	...
	/*
	따라서 C클래스로 인스턴스를 생성하면 오버라이딩 된 메서드는 모두 가려지게 되어 C클래스에 정의한 1개만 남게 된다.
...
public class E07RideAndLoad01 {
	...
	public static void main(String[] args) {
		...
		/*
		부모타입의 참조변수로 자식인스턴스를 참조할 수 있으므로 아래 문장은 모두 정상적으로 실행된다.
		 */
		A ref1 = new C();
		B ref2 = new C();
		C ref3 = new C();
		...
		/*
		B타입으로는 접근 범위가 B클래스까지로 제한되므로 C에 정의도니 메서드는 호출할 수 없다.
		 */
		ref2.loadMethod();
		ref2.loadMethod(1);
		...
		/*
		A타입으로는 접근범위가 A클래스까지로 제한되므로 B와 C에 정의된 메소드는 호출할 수 없다.
		 */
		ref1.loadMethod();
		...
		// A타입 -> C타입을 참조하므로 가능
		A refId1 = new C();
		// B타입 -> A타입을 참조하므로 불가능 (자식은 부모를 참조할 수 없다)
		// B refId2 = refId1;
		// C타입 -> A타입을 참조하므로 불가능 (자식은 부모를 참조할 수 없다)
		// C refId3 = refId1;
		...
		이는 클래스가 상속관계에 있기 때문에 가능하고, 방식은 기본자료형과 동일하게 소괄호를 이용하면 된다.
		 */
		B refId2 = (B) refId1;
		// C refId3 = (B)refId1;
}
