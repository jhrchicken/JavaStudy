...
public class DeChild extends DeParent {
	...
	public DeChild(String name, int age, String stNumber) {
		/*
		자식은 부모의 생성자 호출을 통해 먼저 부모인스턴스를 생성한 후 자식의 인스턴스를 생성해야 한다.
		 */
		super(name, age);
	...
	public void study() {
		/*
		부모의 멤버변수 name은 private이므로 자식에서는 getter를 통해 값을 반환받아야 한다.
		 */
	...
	/*
	오버라이딩 아님 : 부모클래스에서 private 멤버로 선언했으므로 자식쪽에서는 보이지 않아 오버라이딩의 대상이 될 수 없다.
	...
	/*
	메서드 오버라이딩의 조건
	1. 부모클래스에서 선언될 때와 비교해서 접근범위가 동일하거나 그보다 넓으면 가능하다.
	2. 부모에서 default로 선언하면 자식에서는 default 혹은 그보다 넓은 public으로 선언하면 오버라이딩이 성립한다.
	 */
	@Override
	public String sleep() {
	...
	클래스 외부(메서드영역)에 별도로 로드되므로 오버라이딩이 되지 않고 호출 시 각 클래스명을 통해 접근할 수 있다.
	 */
