...
/*
오버라이딩 (Overridinf)
: 클래스가 상속관계에 있을 때 하위클래스에서 상위클래스의 멤버메서드와 완전히 동일한 모양으로 메서드를 재정의하는 것을 말한다.
오버로딩과 다른 점은 매개변수의 갯수 및 타입, 반환타입 등이 완전히 똑같은 형태로 정의하는 것이다.
하위클래스에서 오버라이딩한 메서드는 상위클래스의 메서드를 가리게 되므로 항상 하위 클래스에서 오버라이딩한 메서드가 호출된다.
 */
...
class Speaker {
	...
	// 멤버변수가 private 이므로 외부에서 값을 설정할 수 있도록 정의한 setter 메서드
...
class BaseSpeaker extends Speaker {
	...
	/*
	@Override : 어노테이션이라 부르고, 오버라이딩 된 메서드에 추가하여 컴파일러에게 알려주는 역할을 한다.
	필수사항은 아니므로 생략할 수 있다.
	 */
	@Override
	public void showState() {
		...
		특히 오버라이딩 된 메서드를 호출할 때 super가 없으면 재귀호출이 되므로 주의해야한다.
		 */
		super.showState();
...
public class E06Overriding {
	...
	public static void main(String[] args) {
		...
		윈도우 탐색기에서 같은 이름의 파일을 붙여넣기 할 때 덮어쓸지 물어보는 것과 동일한 원리라 생각하면 된다.
		 */
		...
		/*
		단 부모타입의 변수로 자식 인스턴스를 참조하면 접근할 수 있는 범위가 부모인스턴스까지로 제한되므로,
		자식쪽에 정의된 메서드는 호출할 수 없다.
		 */
