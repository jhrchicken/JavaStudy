...
단지 기본정보를 저장하고 있는 상속을 목적으로 정의된 클래스이다.
...
class HighFriend extends Friend {
	...
	/*
	오버라이딩 1 : 고딩친구의 전체정보를 출력하기 위해 부모에서 정의한 메서드를 super를 통해 먼저 호출하고,
	...
	/*
	오버라이딩 2 : 자식 클래스에서 오버라이딩 하여 재정의한 메서드로 고딩친구의 간략정보를 출력한다.
...
class UnivFriend extends Friend {
	...
	// 오버라이딩 1, 2 역시 High 클래스와 동일한 패턴으로 정의함
	@Override
	public void showAllData() {
...
class FriendInfoHandler {
	...
	부모로 자식을 참조할 수 있으므로 부모타입의 배열에는 자식타입의 인스턴스를 모두 저장할 수 있다.
	 */
	private Friend[] myFriends;
	/*
	배열에 저장된 연락처 정보를 카운트하기 위한 변수로, 추가할 때마다 ++(증가연산자)로 1씩 증가한다.
	 */
	...
	4. 출력을 위한 멤버메서드는 상속관계에서 오버라이딩 되어 있으므로 참조변수의 영향없이 항상 자식클래스에 오버라이딩 된 메서드가 호출된다.
	5. 즉 저장된 인스턴스는 Friend 타입이지만 오버라이딩을 통해 별도의 형변환이 필요하지 않다. 항상 원하는 정보를 간단히 출력할 수 있다.
	 */
	public void showAllData() {
	...
	public void showSimpleData() {
		...
		자식 클래스에서는 오버라이딩을 할 수 없으므로 개별적인 메서드로 정의해야한다.
		정보가 저장되는 배열은 Friend 타입이므로 자식인스턴스를 저장하면 자동으로 형변환(업캐스팅) 된다.
		이때 자식의 정보를 출력하는 메서드를 바로 호출할 수 없으므로 아래와 같이 일일이 확인한 후 강제형변환(다운캐스팅) 해야한다.
	...
	public void searchInfo() {
		...
		for (int i = 0; i < numOfFriends; i++) {
			...
			compareTo 대신 eqauls를 사용해도 결과는 동일하다.
			 */
	...
	public void deleteInfo() {
		...
		/*
		배열의 원소 중 삭제된 것의 인덱스 값을 저장하기 위한 변수로 삭제한 후 빈자리를 채워넣기 위해 사용된다.
		...
		for (int i = 0; i < numOfFriends; i++) {
			...
			if (deleteName.compareTo(myFriends[i].name) == 0) {
				...
				참조값이 null 이라는 것은 참조할 인스턴스가 없다는 의미이므로 삭제한 것과 동일한 결과가 된다.
				실제로 참조되지 않는 인스턴스는 "가비지 컬렉션"에 의해 자동으로 삭제된다.
				 */
				myFriends[i] = null;
				// 삭제된 원소의 index를 저장한다.
				deleteIndex = i;
		...
		if (deleteIndex == -1) {
			...
			/*
			인스턴스 배열에서 원소를 삭제한 후 바로 뒷부분에 있는 원소를 하나씩 앞으로 복사한다.
...
// 메인클래스
public class E10FriendInfoBook {
	...
	/*
	메인 메서드는 해당프로그램의 시작점(Entry point)이므로 복잡한 로직의 구성보다는 프로그램의 전반적인 흐름에 대해서만 기술하는 것이 좋다.
	...
	public static void main(String[] args) {
		...
		break문은 반복문을 탈출시키는 기능이 있으므로 이와 같은 무한루 에서 자주 사용된다.
		for(;;) 문을 통해 무한루프를 구현할 수 있으나 반복의 횟수가 명확하지 않은 경우에는 주로 while 문을 사용한다.
		 */
		while (true) {
			// 1. 메뉴를 출력
			menuShow();
