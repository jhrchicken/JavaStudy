...
class Computer {
	...
	/*
	private 접근지정자로 선언된 멤버메서드는 외부 클래스 혹은 상속받은 클래스에서 접근이 가능하므로,
	...
	/*
	private으로 선언된 멤버메서드를 외부에서 호출할 수 있도록 public으로 지정하여 선언하였다.
...
class NotebookComputer extends Computer {
	...
	public void movingCal() {
		...
		if (battary < 1) {
			System.out.println("배터리가 방전되어 사용 불가합니다.");
			/*
		...
		/*
		아래 2개의 메서드는 private으로 선언되었으므로 상속을 받은 자식클래스에서도 접근할 수 없어 에러가 발생한다.
...
/*
태블릿 노트북 : 컴퓨터의 기본 연산기능과, 휴대이용 그리고 전용펜으로 드로잉 할 수 있는 기능까지 추가된 컴퓨터
 */
class TabletNotebook extends NotebookComputer {
	...
	public void write(String penInfo) {
		...
		A.compareTo(B) : 문자열 A와 B를 비교하여 0이 반환되면 동일한 문자열,
			-1 혹은 1이 반환되면 다른 문자열로 판단하는 String 클래스에 정의된 메소드
		 */
		...
		상속받은 자식클래스에서는 이와 같이 부모의 멤버메서드를 호출하여 자신의 멤버처럼 사용할 수 있다.
