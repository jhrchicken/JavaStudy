...
public class E01OneDimArray02 {
	...
	public static void main(String[] args) {
		...
		난수 생성 방법 2
		: Random 클래스를 사용한다.
		사용법은 Scanner와 동일하게 정수형 난수를 생성하고 싶다면 nextInt()를 사용한다.
		 */
		Random random = new Random();
		System.out.println("셍성된 난수 : " + random.nextInt());
		System.out.println("================");

		/*
		로또번호처럼 1~45 사이의 난수를 생성하는 방법
		1. 0.xxx 형태의 난수를 생성한 후 정수를 변경하기 위해 100을 곱한다
		2. 45로 % 연산하여 나머지를 구한다. 정수를 45로 나누면 나머지는 0~44 사이가 된다.
		3. 0은 구간에 포함되지 않으므로 결과에 1을 더해준다.
		4. 정수의 결과가 필요하므로 int로 강제형변환 한다.
		 */
		System.out.println("1 ~ 45까지의 난수 생성 : " + ((int) ((Math.random() * 100) % 45)) + 1);
		System.out.println("================");
		
		/*
		크기가 6인 배열을 생성하여 난수를 저장한 후 출력한다.
		...
		for (int i = 0; i < lottoNum.length; i++) {
			lottoNum[i] = (int) ((Math.random() * 100) % 45);
		}
		// 순서대로 출력
		for (int i = 0; i < lottoNum.length; i++) {
