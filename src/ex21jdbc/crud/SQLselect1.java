...
public class SQLselect1 extends MyConnection {
	...
	public void dbExecute() {
		try {
			...
			// 정적쿼리문 작성. to_char 함수를 통해 날짜의 서식을 지정한 컬럼은 별칭을 부여함
			...
			조건에 맞는 레코드가 반환되므로 ResultSet 타입의 인스턴스를 반환값으로 return 된다.
			 */
			rs = stmt.executeQuery(query);

			/*
			반환된 레코드의 개수만큼 반복한다.
			조건에 따라 개수는 달라질 수 있으므로 while문을 주로 사용한다.
			next() 메서드는 인출할 레코드가 있는지를 판단하여 true/false를 반환한다.
			 */
			while (rs.next()) {
				// select 절의 첫번째 컬럼이므로 id 컬럼이 지정된다.
