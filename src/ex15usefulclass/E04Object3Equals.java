...
class MyClass {
	...
	/*
	MyClass 인스턴스 간의 내용 비교를 위해 equals()를 오버라이딩 하여 재정의한다.
	인스턴스가 가진 멤버변수의 값에 대하나 비교를 통해 동일여부를 판단한다.
	equals() 메서드의 매개변수 타입은 모든 인스턴스를 대상으로 하므로 Object형으로 정의되어 있다.
	 */
	@Override
	public boolean equals(Object obj) {
		/*
		매개변수로 전달된 인스턴스를 Object로 받으면 업캐스팅이 되므로 다운캐스팅 이후에 비교를 진행해야 한다.
		이때 전달된 인스턴스가 MyClass타입이 아니라면 비교의 대상이 될 수 없으므로 상속관계 확인을 위해 instanceof 연산자를 사용한다.
		 */
		if (obj instanceof MyClass) {
			...
			/*
			매개변수로 전달된 인스턴스가 MyClass타입이 아니라면 비교의 대상이 될 수 없으므로 false를 반환한다.
			 */
...
public class E04Object3Equals {
	...
	public static void main(String[] args) {
		...
		System.err.println("[두 객체를 비교연산자를 통해 비교]");
		...
		즉 오버라이딩이 필요없는 경우는 기본클래스일 때만 가능하다.
		 */
		System.err.println("[두 객체를 equals() 메서드로 비교");
		System.out.println(mc1.equals(mc2) ? "같다" : "다르다"); // 같다

		/*
		print()를 통해 출력하면 자동으로 toString()을 호출하므로 아래 출력결과는 동일하다.
		 */
		System.err.println("[두 객체의 toString()메서드 호출]");
