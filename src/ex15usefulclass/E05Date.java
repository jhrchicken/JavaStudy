...
public class E05Date {
	...
	public static void main(String[] args) {
		...
		/*
		월을 표현하는 Month, 분을 표현하는 Minute의 이니셜이 동일하므로 구분을 위해 월은 대분자, 분은 소문자를 사용한다.
		 */
		String nowStr1 = myDataFormat(calendar, "yyyy-MM-dd HH:mm:ss");
		System.out.println("현재 1 : " + nowStr1);
		// 그 외 여러가지 서식문자가 존재한다.
		String nowStr2 = myDataFormat(calendar, "오늘은 M월의 W번째주, d번째날, F번째 E요일");
		System.out.println("현재 2 : " + nowStr2);

		/*
		현재 날짜를 기준으로 전/후를 구하기 위해 add()메서드를 사용한다.
		음수를 이용하면 과거의 날짜를 구할 수 있다.
		 */
		...
		String nowStr3 = myDataFormat(calendar, "yyyy년 MM월 dd일");
	...
	public static long myDiffDate(String today, String future) {
		...
		try {
			/*
			매개변수를 통해 전달된 두 개의 문자열은 날짜의 서식이 서로 다르다.
			...
			/*
			getTime()으로 얻어오는 시간이 13자리인 정수값인데 10자리까지는 순수한 초(sec)이고, 11~13자리는 밀리세컨즈를 의미한다.
			즉 100으로 나누에서 뒤의 3자리를 제거하면 순수한 '초'가 된다.
