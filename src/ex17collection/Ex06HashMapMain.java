...
- Key 값은 중복될 수 없다. 만약 중복되면 기존의 데이터를 덮어쓰기 한다.
...
public class Ex06HashMapMain {
	...
	public static void main(String[] args) {
		 ...
		 */
		// 처음 저장이므로 null이 반환된다.
		System.out.println("name이라는 키값으로 저장된 이전의 값 : " + map.put("name", "홍길동")); // null
		...
		// 중복저장 되므로 4개가 출력된다.
		System.out.println("키값으로 중복저장 후 객체수 : " + map.size());

		/*
		출력하기 
		1. 키값을 알고 있을 때 : get(key값) 으로 출력한다.
		 */
		System.out.println("키 값을 알때 : " + map.get("name")); // 최길동
		...
		/*
		3. 이터레이터 사용 : 이터레이터를 통해 출력할 때도 KeySet()을 통해 key를 먼저 얻어온 후 반복 출력한다.
		 */
		System.out.println("[이터레이터 사용하기");
		// Map의 전체 key를 얻어온 후 ..
		Set<String> keys2 = map.keySet();
		// 얻어온 key를 통해 이터레이터 인스턴스응 생성한다.
		 ...
		 */
		System.out.println("삭제된 객체 : " + map.remove("age")); // 20
		System.out.println("[age값을 삭제 후 출력]");
		for (String key : keys) {
