...
public class Ex03ArrayList2 {
	...
	public static void main(String[] args) {
		...
		*/
		...
		반환할 인스턴스가 있는지 확인한 후 true면 next()로 출력한다.
		 */
		...
		/*
		시나리오] 컬렉션에 저장된 인스턴스를 이름으로 검색하여 삭제하는 코드를 작성하세요.
		이때 반복문은 확장 for문을 사용하세요.
		 */
		Scanner scanner = new Scanner(System.in);
		System.out.print("삭제할 이름을 입력하세요: ");
		String removeName = scanner.nextLine();
		// 리스트에 저장된 인스턴스의 개수만큼 반복
		for (Student st : list2) {
			/*
			이름으로 검색 시 멤버변수 name은 private으로 선언되었으므로 getter를 통해 접근해야 한다.
			삭제를 위해 입력한 이름과 Student인스턴스에 저장된 name과 일치하는지 확인한 후 해당 인스턴스를 찾을 수 있다.
			 */
