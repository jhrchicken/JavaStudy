...
- set의 기본적인 특성은 동일하다.
- 따라서 중복은 허용하지 않는다.
- 단 인스턴스가 정려로디어 저장된다. 정렬의 기준은 개발자가 직접 정의할 수 있다.
 */

/*
클래스 생성 시 정렬의 기준을 만들어주기 위해서는 Comparable<T> 인터페이스를 구현해야 한다.
 */
class MyString implements Comparable<MyString> {
	...
	정렬의 기준은 단순하게 가나다와 같은 순서가 될 수도 있고, 문자열의 길이에 따른 정렬이 될 수도 있다.
	...
	public int compareTo(MyString paramStr) {
		...
		/*
		if (str.charAt(0) > paramStr.str.charAt(0)) {
			return 1;
		} else {
			return -1;
		}
...
public class Ex05TreeSetMain {
	...
	public static void main(String[] args) {
		...
		/*
		자바에서 제공하는 기본클래스인 경우 별도의 오버라이딩 없이도 정렬되어 저장된다.
		 */
