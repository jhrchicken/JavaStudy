...
class Singer {
	...
	Java에서 정의하는 모든 클래스는 Object클래슬르 상속하므로 아래와 같이 오버라이딩이 가능하다.
	 */
	@Override
...
public class E02AnonymousClass {
	...
	public static void main(String[] args) {
		...
		이때 오버라이딩한 toString()을 삭제하면 @를 포함한 참조값이 출력된다.
		 */
		System.out.println(s1);

		// 자식클래스를 통해 인스턴스를 생성 후 정보출력
		Singer s2 = new GirlGroup("츄", "이달의소녀");
		// 자식쪽의 toString()이 호출되어 이름과 팀명까지 출력한다.
		System.out.println(s2);

		/*
		GirlGroup 클래스를 상속받은 익명클래스 정의
		: Singer 타입의 변수에 GirlGroup을 상속받은 익명클래스를 생성해서 그 참조값을 할당한다.
		개념적으로 new GirlGroup(){};
		...
		접근하고 싶다면 다운캐스팅(강제형변환)을 해야하지만 익명클래스는 이름이 없기 때문에 다운캐스팅 자체가 불가능하다.
		...
		/*
		오버라이딩은 부모에서 생성한 메서드를 자식에서 재정의하는 것으로 애초에 기능이 있는 메서드가 필요하지 않다.
		따라서 인터페이스에 정의된 추상메서드를 오버라이딩하여 익명클래스를 만드는 경우가 매우 많다.
		 */
