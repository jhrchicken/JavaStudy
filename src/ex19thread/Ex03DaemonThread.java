...
: 메인쓰레드와 Working 쓰레드(개발자가 정의한 쓰레드)를 일컫는 것으로 메인쓰레드가 종료되어도 계속 실행되며,
Dead 상태가 되어야만 실행이 종료되는 쓰레드를 말한다.
...
class NormalThread extends Thread {
	...
	/*
	Thread 클래스의 생성자 호출 시 전달하는 인수를 통해 해당 쓰레드의 이름을 지정할 수 있다.
	...
	/*
	쓰레드 동작 중 비정상적으로 종료될 때 InterruptedException이 발생될 수 있으므로, 예외처리를 해야한다.
	여기서는 즉시 처리하지 않고 throws 해서 호출한 위치로 예외 인스턴스를 던지기한다.
	 */
...
메인메서드도 하나의 독립쓰레드이므로, 다른 독립쓰레드가 남아 있다면 종료될 때까지는 프로그램이 종료된 것이 아니다.
...
public class Ex03DaemonThread {
	...
	public static void main(String[] args) {
		...
		쓰레드 인스턴스 생성 시 이름을 부여하는 방법으로 아래 두 가지가 있다.
		 */
