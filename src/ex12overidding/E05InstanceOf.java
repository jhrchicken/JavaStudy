...
/*
instanceof 연산자
: 인스턴스 변수가 어떤 타입의 변수인지를 판단하는 연산자로 형변환(즉 상속관계가 있으면)이 가능하면 true를 반환한다.
- 인스턴스 간에 형변환이 되려면 반드시 두 클래스 간에 상속관계가 있어야 한다.
- 부모타입의 참조변수로 자식 인스턴스를 참조할 수 있으므로 매개변수를 부모타입으로 정의하면 자식 인스턴스를 인수로 받을 수 있다.
- 매개변수로 전달되는 인수는 부모타입으로 자동형변환(업캐스팅)되므로 정상적인 사용을 위해 강제형변환(다운캐스팅)해야 하는데 이때 확인을 위해 사용된다.
 */
...
public class E05InstanceOf {
	...
	- instanceof 연산자를 통해 자식타입으로 형변환이 가능한지 판단한 후 true를 반환하면 다운캐스팅을 진행한 후 메서드를 호출할 수 있다.
	 */
	static void wrapBox(Box b) {
		/*
		세가지 인스턴스 중 어떤 것이 전달되더라도 Box타입으로 자동형변환 되므로 해당 매개변수를 통해 호출할 수 있는 메서드는 boxWrap()이 유일하다.
		따라서 다운캐스팅이 필요하다.
		 */
	...
	/*
	만약 부모타입의 매개변수로 자식 인스턴스를 받을 수 없다면, 아래와 같이 각각의 타입에 따른 모든 메서드를 정의해야 한다.
