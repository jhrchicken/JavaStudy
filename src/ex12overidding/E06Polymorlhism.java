...
/*
다형성 (Polymorphism)
: 하나의 인스턴스(객체)를 여러 타입의 참조변수를 통해 참조할 때 참조변수 타입에 따라 다양한 형태의 인스턴스를 이용할 수 있는 특성을 말한다.
...
2. 이것을 동질화 (Homoreneous : 호모지니어스)라고 표현한다.
 */
...
class MyChild extends MyParent {
	...
	@Override
	void parentMethod() {
		System.out.println("자식에서 Overridng한 메소드 : parentMethod()");
	}
	...
	void parentMethod(int childMember) {
		...
		System.out.println("Overriding : 자식에서 확장한 메소드 : parentMethod(param1)");
	}
}

public class E06Polymorlhism {
	...
	public static void main(String[] args) {
		...
		이질화 : 부모타입의 참조변수로 자식인스턴스를 참조하는 것을 말한다.
			...
			만약 접근해야 한다면 자식타입으로 강제형변환(다운캐스팅)을 해야 한다.
		 */
		...
		또한 Object 클래스에 정의된 모든 메서드를 별도의 처리없이 사용할 수 있고, 
		필요하다면 오버라이딩도 할 수 있다.
		*/
