...
public class PersonConstructor {
	...
	/*
	개발자가 생성자를 직접 정의하지 않으면 아래와 같이 매개변수와 실행부가 없는 '디폴트 생성자'가 컴파일러에 의해 자동으로 삽입된다.
	...
	void initialize(String name, int age, String addr) {
		...
		생성자는 인스턴스 생성 시 딱 한번만 호출되고, 개발자가 임의로 호출할 수 없다.
		하지만 멤버메서드는 개발자가 원할 때 언제든 호출이 가능하므로 생성자와는 특성이 다르다.
