...
- 또한 Statck은 CPU가 메모리의 생성 및 소멸을 주관한다.
 */
...
public class E06LocalVariable {
	...
	public static void main(String[] args) {
		...
		/*
		main 지역에서 num을 선언하면 그보다 좁은 지역인 if 문에서는 같은 이름의 변수를 선언할 수 없다.
		큰 지역에서 이미 선언되어 메모리에 상주하고 있기 때문이다.
		 */
		// int num = 9;

		/*
		main보다 좁은 지역인 if문의 블럭을 생성한 후 num을 선언한다.
		...
		// if 문의 블럭에서 선언된 변수 num은 해당 블록을 벗어나는 순간 메모리에서 소멸되므로 아래에서 num을 새롭게 선언할 수 있다.
		int num = 100;
	...
	/*
	main 지역에서 선언한 변수 num은 메모리에 아직 상주하고 있지만 해당 메서드는 크기가 동일한 다른 지역에 정의되어있으므로 같은 이름의 변수를 선언할 수 있다.
