...
public class Ex06DeveloperDefine {
	...
	public static void main(String[] args) {
		...
		try {
			/*
			아래 메서드에서 AgeErrorException 예외 인스턴스를 throw하면 여기에서 예외처리한다.
	...
	public static int readAge() throws AgeErrorException {
		...
		try {
			/*
			나이 입력 시 문자를 입력하는 경우에 대해 예외처리를 하고 있다.
		...
		/*
		만약 나이를 음수로 입력하는 경우 JVM이 감지하지 못하므로 if문을 통해 직접 판단해야 한다.
		 */
		if (inputAge < 0) {
			/*
			...
			이는 프로그램의 종료로 이어지므로 반드시 예외처리를 해야한다.
			여기서는 직접 예외처리를 하지 않고 호출한 main() 메서드로 인스턴스를 throws 한다.
			 */
		}
		return inputAge;
	}
