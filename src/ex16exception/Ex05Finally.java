...
public class Ex05Finally {
	...
	/*
	런타임 예외가 발생하는 메서드로 예외가 발생하면 호출한 지점으로 예외던지기를 한다.
	즉 예외를 무시한다.
	 */
	...
	static void tryCatchFinally() {
		...
		try {
			...
			/*
			try절에서 예외발생과 상관없이 또는 return을 만나더라도 항상 finally절은 실행된다.
			 */
	...
	public static void main(String[] args) {
		...
		/*
		NumberFormatException과 같은 예외는 발생된 지점에서 예외던지기를 하는 것은 가능하나,
		반드시 예외처리 해야만 프로그램이 종료되지 않는다.
		즉 예외 인스턴스에 따라 무시할 수 있는 것도 있지만, 이처럼 반드시 예외처리를 해야하는 것도 존재한다.
		 */
		try {
			runtime();
		...
		/*
		프로그램 내에서 예외가 발생되었이만 예외처리를 통해 실행의 흐름이 끝까지 이어지므로 해당 문장이 실행된다.
