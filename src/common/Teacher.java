...
public class Teacher extends Person {
	...
	/*
	시나리오] Teacher 객체는 나이와 과목이 같으면 동일한 객체로 처리하도록 hashCode와 equals 메서드를 적절히 오버라이딩 하세요.
	 */
	...
	public int hashCode() {
		/*
		방법 1 : 정수형변수와 인스턴스의 해시값을 합산한 후 그대로 반환한다.
		 */
		int returnCode1 = super.getAge() + this.subject.hashCode();
		// System.out.println(returnCode1);

		/*
		방법 2 : 방법 1이 번거롭다면 Objects 클래스의 정적 메서드인 hash()를 사용해도 된다.
			멤버변수의 개수만큼 인수로 전달하면 위와 동일하게 고유한 주소값을 정수형으로 반환해준다.
		 */
		int returnCode2 = Objects.hash(super.getAge(), this.subject);
		// System.out.println(returnCode2);
	...
	값이 같을 때는 true 다를 때는 false를 반환하도록 오버라이딩 하면 된다.
	 */
	@Override
